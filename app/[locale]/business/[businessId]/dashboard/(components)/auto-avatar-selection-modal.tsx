...
import { AutoGenerateAvatarOption } from "@/contexts/auto-generate-context";
...
interface AutoAvatarSelectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (item: AutoGenerateAvatarOption | null) => void;
  selectedAvatar: AutoGenerateAvatarOption | null;
}

export function AutoAvatarSelectionModal({
  isOpen,
  onClose,
  onSave,
  selectedAvatar,
}: AutoAvatarSelectionModalProps) {
  ...
  const [tempSelected, setTempSelected] = useState<AutoGenerateAvatarOption | null>(
    null
  );
  ...
  const businessAvatarOptions = useMemo<AutoGenerateAvatarOption[]>(() => {
  ...
  const browseAvatarOptions = useMemo<AutoGenerateAvatarOption[]>(() => {
  ...
  const toggleAvatar = (item: AutoGenerateAvatarOption) => {
    setTempSelected((current) => (current?.id === item.id ? null : item));
  };
